...
(async function () {
    ...
    if (!ENABLED) {
        ...
        // Find the subtitle button, replace its icon, and add our direct click listener
        const buttonObserver = new MutationObserver(function (mutationsList, observer) {
            ...
            if (button) {
                console.info('[Buttercup] Found subtitle button, attaching listener.');
                observer.disconnect(); // Stop observing once the button is found

                button.innerHTML = window.trustedTypes.defaultPolicy.createHTML(SVG_BCAPTIONS);

                // Add a direct click listener to start our process
    ...
    const interval = setInterval(() => {
        if (window['ytInitialPlayerResponse']) {
            if (ENABLED) { // Only inject if Buttercup is enabled
                console.info('[Buttercup] Injecting Buttercup captions object into initial player response');
                window['ytInitialPlayerResponse'].captions = CAPTIONS_OBJECT;
            }
            clearInterval(interval); // Stop checking once player response is available and handled
        }
    }, 1);
    ...
    function startTranscriptionProcess() {
        ...
        transcriptionHandler.processVideo(
            ...
            (youtubeFormat) => {
                ...
                setLoading(false);
                // We need to click the button again to make YouTube re-render the subtitles menu with our new track
                clickSubtitleButton();
            },
            // Error callback
            (error) => {
